...
class StatusNarrator:
    ...
    def get_status(
        ...
        return StatusNarrative(
            timestamp=datetime.now(),
            current_objective=current_objective or (self.objectives[-1] if self.objectives else None),
    ...
    def _update_max_risk(self, risk_level: RiskLevel) -> None:
        ...
        if self.max_risk_seen is None:
            self.max_risk_seen = risk_level
        elif risk_order.get(risk_level, -1) > risk_order.get(self.max_risk_seen, -1):
            self.max_risk_seen = risk_level
